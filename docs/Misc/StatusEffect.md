...
| OnImpact         | Executes when the entity hits something hard enough. For items, the threshold is determined by ,<br/>for characters by . Valid for items and characters.                                              |
...
| OnProduceSpawned | Executes when a  produces an item (e.g. when a plant grows a fruit). Only valid for Growable items.                                                                                                   |
| OnOpen           | Executes when a  is opened. Only valid for doors.                                                                                                                                                     |
| OnClose          | Executes when a  is closed. Only valid for doors.                                                                                                                                                     |
...
| NearbyCharacters | Characters near the entity the StatusEffect is defined in. The range is defined using .                                             |
| NearbyItems      | Items near the entity the StatusEffect is defined in. The range is defined using .                                                  |
| UseTarget        | The entity the item/attack is being used on.                                                                                        |
| Hull             | The hull the entity is inside.                                                                                                      |
| Limb             | The entity the item/attack is being used on. In the context of characters, one of the character's limbs (specify which one using ). |
...
| OnlyPlayerTriggered              | bool       | See description | If enabled, the effect only executes when the entity receives damage from a player character<br/>(a character controlled by a human player). Only valid for characters, and effects of the type .                                                                                                                                                                                                                                                                                  |
